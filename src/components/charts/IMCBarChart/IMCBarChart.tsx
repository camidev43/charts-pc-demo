...
export default function IMCBarChart({ expanded }: { expanded?: boolean }) {
  ...
  const option = {
    ...
    xAxis: {
      ...
      axisLabel: { color: cc.text, fontSize: expanded ? 12 : 9, interval: expanded ? 0 : 1 },
    },
    yAxis: {
      type: 'value',
      min: 25, max: 32,
        ...
        itemStyle: {
          ...
          color: (params: { dataIndex: number }) => {
            ...
            if (val >= 29) return new echarts.graphic.LinearGradient(0, 0, 0, 1, [
              { offset: 0, color: '#FF3B30' }, { offset: 1, color: '#FF9A93' },
            ]);
            if (val >= 27.5) return new echarts.graphic.LinearGradient(0, 0, 0, 1, [
              { offset: 0, color: '#FBBF24' }, { offset: 1, color: '#FFE08A' },
            ]);
            return new echarts.graphic.LinearGradient(0, 0, 0, 1, [
              { offset: 0, color: '#30B0C7' }, { offset: 1, color: '#8FD6E0' },
            ]);
          },
        },
      },
